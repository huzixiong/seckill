...
def get_seckill_result_by_log():
    ...
    """
    keyword = "抢购成功"
    file_path = jd_logger.LOG_FILENAME
...
if __name__ == '__main__':
    # 设置发送微信消息
    if global_config.getRaw('messenger', 'sckey'):
        message = get_seckill_result_by_log()
        send_wechat(title=message[0], message=message[-1])
        jd_logger.logger.info("抢购结果已发送至微信，请注意查收！")
    else:
        jd_logger.logger.info("发送失败，请在 config.ini 内配置 sckey ！")

...
class JdSeckill(object):
    ...
    def check_login(func):
        ...
        def new_func(self, *args, **kwargs):
            if not self.qrlogin.is_login:
                logger.info("当前未登录，调用 {0}() 失败，请扫码登录！".format(func.__name__))
    ...
    def seckill_by_proc_pool(self, work_count=5):
        ...
        logger.info('用户名称:【{}】'.format(self.get_username()))
        # logger.info('商品名称:{}'.format(self.get_sku_title()))
        if self.timers.local_time_greater_than_buy_time():
            logger.info("今天抢购时间已过，明天再来吧～")
        else:
    ...
    def make_reserve(self):
        ...
        # self.timers.start()  # 预约不需要等待到指定时间，所以注释掉
        while True:
            try:
                r = self.session.get(url='https:' + reserve_url)
                logger.info(f"预约成功：{r.url}")
                success_message = "预约成功，已获得抢购资格 / 您已成功预约过了，无需重复预约"
                logger.info(success_message)
                if global_config.getRaw('messenger', 'enable') == 'true':
                    send_wechat(success_message, title="预约成功")
                break
            except Exception as e:
                logger.error(f'预约失败正在重试...{e}')
            time.sleep(1)
    ...
    def get_username(self):
        ...
        # jQuery2381773({"imgUrl":"//storage.360buyimg.com/i.imageUpload/xxx.jpg","lastLoginTime":"","nickName":"xxx",
        # "plusStatus":"0","realName":"xxx","userLevel":x,"userScoreVO":{"accountScore":xx,"activityScore":xx,
        # "consumptionScore":xxxxx,"default":false,"financeScore":xxx,"pin":"xxx","riskScore":x,"totalScore":xxxxx}})
    ...
    def get_seckill_url(self):
        ...
        while True:
            ...
            if resp_json.get('url'):
                ...
                logger.info("准备开始抢购，提前 [500] 毫秒开始发送请求...")
                wait_some_time()

    def request_seckill_url(self):
        """访问商品的抢购链接（用于设置cookie等）
        """
        # 5个进程同时去获取用户名和商品名称，有点冗余，运行前获取一次就可以了
        # logger.info('用户:{}'.format(self.get_username()))
        # logger.info('商品名称:{}'.format(self.get_sku_title()))
        self.timers.start()
        self.seckill_url[self.sku_id] = self.get_seckill_url()
        logger.info('获取商品的抢购链接...')
        headers = {
            'User-Agent': self.user_agent,
    ...
    def submit_seckill_order(self):
        ...
        try:
            ...
            if e == "addressList":  # 该地区无货
                logger.info("当前地区暂时无货，请登录 APP 查看详情.")
            else:
